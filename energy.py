...
class Optimizer(object):
    ...
    def EnergyCalculator(self, sites, offsets, labelling):
        i, e = 0, 0
        while i < len(sites) and e < np.inf:
            e = e + D(sites[i], offsets[labelling[i]])
            neighbors = GetNeighbors(sites[i])
            for n in neighbors:
                if n in sites:
                    if sites.index(n) > i:
    ...
    def GetNeighbors(self, site):
        return [[site[0]-1, stite[1]], [site[0], stite[1]-1], [site[0]+1, stite[1]], [site[0], stite[1]+1]]
